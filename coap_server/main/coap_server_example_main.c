...
float get_actual_temperature(float min, float max){

    float temperature_range = max - min;
    float scaled = temperature_range / RAND_MAX;
    float result = (float)rand() * scaled + min;
    return result;
}

void save_actual_temperature(){

    float data = get_actual_temperature(0.0, 10.0);
    sprintf(temperature_data, "%.1f", data);
    printf("Actual temperature: %s\n", temperature_data);
                  ...
                  coap_pdu_t *response)
{
    ...
    coap_add_data_large_response(resource, session, request, response,
                                 ...
                                 (const u_char *)espressif_data, /*Temperature function*/
                                 NULL, NULL);
}

/*
 * The resource handler for simulated temperature
 */
static void
hnd_temp_exterior_get(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    /*Save current simulated temperature*/
    srand(time(NULL));
    save_actual_temperature();

    size_t temperature_len = strlen(temperature_data);

    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)temperature_len,
                                 (const u_char *)temperature_data, /*Temperature function*/
                                 NULL, NULL);
}
...
{
    /*CoAP context and serv address*/
    coap_context_t *ctx = NULL;
    coap_address_t serv_addr;

    /*List of temperature resources*/
    coap_resource_t *resource_frijol = NULL;
    coap_resource_t *resource_zan = NULL;
    coap_resource_t *resource_calabaza = NULL;
    coap_resource_t *resource_maiz = NULL;

    /*INITIALIZE MDNS SERVER*/
    mdns_init();
    mdns_hostname_set("ESP32-TEMP-LINK");

    /*mDNS basic text item structure*/
    /*Used in mdns_service_add()*/
    mdns_txt_item_t serviceTxData[3] = {
        {"TMP-MODULE", "esp32"},
        {"TMP-MONITOR", "user"},
        {"1234567890", "password"}  
    ...
    /*mDNS configuration: Host Name, protocol, port and TX data*/
    mdns_service_add("ESP32-TEMP-LINK", "_coap", "_udp", 80, serviceTxData, 3);
    ...
    while (1) {
        ...
        /*TEMP - FRIJOL*/
        resource_frijol = coap_resource_init(coap_make_str_const("temp/frijol"), 0);
        if (!resource_frijol) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_frijol, COAP_REQUEST_GET, hnd_temp_exterior_get);
        coap_register_handler(resource_frijol, COAP_REQUEST_PUT, hnd_espressif_put);
        coap_register_handler(resource_frijol, COAP_REQUEST_DELETE, hnd_espressif_delete);
        /* We possibly want to Observe the GETs */
        coap_resource_set_get_observable(resource_frijol, 1);
        coap_add_resource(ctx, resource_frijol);
        
        /*TEMP - ZANAHORIA*/
        resource_zan = coap_resource_init(coap_make_str_const("temp/zan"), 0);
        if (!resource_zan) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_zan, COAP_REQUEST_GET, hnd_temp_exterior_get);
        coap_register_handler(resource_zan, COAP_REQUEST_PUT, hnd_espressif_put);
        coap_register_handler(resource_zan, COAP_REQUEST_DELETE, hnd_espressif_delete);
        /* We possibly want to Observe the GETs */
        coap_resource_set_get_observable(resource_zan, 1);
        coap_add_resource(ctx, resource_zan);

        /*TEMP - CALABAZA*/
        resource_calabaza = coap_resource_init(coap_make_str_const("temp/calabaza"), 0);
        if (!resource_calabaza) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_calabaza, COAP_REQUEST_GET, hnd_temp_exterior_get);
        coap_register_handler(resource_calabaza, COAP_REQUEST_PUT, hnd_espressif_put);
        coap_register_handler(resource_calabaza, COAP_REQUEST_DELETE, hnd_espressif_delete);
        /* We possibly want to Observe the GETs */
        coap_resource_set_get_observable(resource_calabaza, 1);
        coap_add_resource(ctx, resource_calabaza);

        /*TEMP - MAIZ*/
        resource_maiz = coap_resource_init(coap_make_str_const("temp/maiz"), 0);
        if (!resource_maiz) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource_maiz, COAP_REQUEST_GET, hnd_temp_exterior_get);
        coap_register_handler(resource_maiz, COAP_REQUEST_PUT, hnd_espressif_put);
        coap_register_handler(resource_maiz, COAP_REQUEST_DELETE, hnd_espressif_delete);
        /* We possibly want to Observe the GETs */
        coap_resource_set_get_observable(resource_maiz, 1);
        coap_add_resource(ctx, resource_maiz);
